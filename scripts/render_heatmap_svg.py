...
contribution heatmap SVG in Sajid's plum/coral theme -- a grid of rounded
boxes in the classic 53-week x 7-day calendar, revealed once with a
diagonal line-after-line slide-down, a Less->More legend, and a stats
footer.

This is a reskin, not a copy: original used a GitHub-green ramp on a
blue-bordered dark-navy card labeled "avi@github". This version uses
Sajid's existing theme -- #2B2738 plum background, #FF6E6E coral ramp,
#7FFFD4 mint accents, white border -- matching his other embeds
(aura_dark stat cards) instead of introducing a new, unrelated palette.
...
# Sajid's plum -> coral ramp: empty -> brightest, matching bg_color/title_color
# from his existing github-readme-stats embeds instead of GitHub green.
PALETTE = ["#211E2B", "#4A2E3A", "#7A3548", "#B33F55", "#E85463", "#FF6E6E"]
...
FRAME = "#FFFFFF"      # Sajid's existing border_color
MUTED = "#A9A4BC"
TEXT = "#EAEAEA"       # Sajid's existing text_color
ACCENT = "#7FFFD4"     # Sajid's existing icon_color
CORAL = "#FF6E6E"      # Sajid's existing title_color
GOLD = "#f2cc60"
...
def render(data, username="Zlmaoooo"):
    days = data["days"]
...
""".strip()
    ...
    parts.append(f'<text x="{canvas_w/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
                 f'text-anchor="middle">sajid@nexus: ~/contributions --graph</text>')
